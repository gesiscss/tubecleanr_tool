...
simulated_yt <- read_csv2("data/simulated_yt.csv", locale = locale(encoding = "UTF-8"))
```
...
Please note that -- just like the comments -- the video IDs, URLs and user names in the data are fake (or simulated if you prefer a nicer word).
...
processed_yt <- parse_yt_comments(simulated_yt,
                                  package = "vosonSML") # here, we specify that the data was collected with the vosonSML package
```
